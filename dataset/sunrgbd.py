...
# ============================================================================
"""Groupfree 3D SUN RGB-D Dataset"""
...
from transforms import pc_util
...
class SunrgbdDetectionVotesDataset():
    ...
    def __getitem__(self, idx):
        ...
        return point_cloud.astype(np.float32), target_bboxes.astype(np.float32)[:, 0:3], angle_classes.astype(np.int64), angle_residuals.astype(np.float32), size_classes.astype(np.int64), size_residuals.astype(np.float32), size_gts.astype(np.float32), target_bboxes_semcls.astype(np.int64), target_bboxes_mask.astype(np.float32), point_obj_mask.astype(np.int64), point_instance_label.astype(np.int64), np.array(idx).astype(np.int64), max_bboxes


if __name__ == '__main__':
    data_root = '/home/tyolm/repos/Group-Free-3D'
    dataset = SunrgbdDetectionVotesDataset(data_root=data_root)
    dataset_generator = ds.GeneratorDataset(dataset, ["point_clouds", "center_label", "heading_class_label", "heading_residual_label", "size_class_label", "size_residual_label", "size_gts", "sem_cls_label", "box_label_mask", "point_obj_mask", "point_instance_label", "scan_idx", "max_gt_bboxes"])

    dataset_generator_batch = dataset_generator.batch(batch_size=4)
