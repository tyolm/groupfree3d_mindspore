...
from groupfree_3d_eval_det import get_iou_obb, eval_det_multiprocessing
...
def parse_predictions(end_points, config_dict, prefix="", size_cls_agnostic=False):
    ...
    pred_heading_class = ops.Argmax()(end_points[f'{prefix}heading_scores'])  # B,num_proposal
    pred_heading_residual = ops.GatherD()(end_points[f'{prefix}heading_residuals'], 2, ops.ExpandDims()(pred_heading_class, -1))  # B,num_proposal,1
    ...
    if size_cls_agnostic:
        ...
        pred_size_class = ops.Argmax()(end_points[f'{prefix}size_scores'])  # B,num_proposal
        pred_size_residual = ops.GatherD()(end_points[f'{prefix}size_residuals'], 2, mnp.tile(ops.ExpandDims()(ops.ExpandDims()(pred_size_class, -1), -1), (1, 1, 1, 3)))  # B,num_proposal,1,3
        # pred_size_residual.squeeze_(2)
        pred_size_residual = pred_size_residual.squeeze(2)

    pred_sem_cls = ops.Argmax()(end_points[f'{prefix}sem_cls_scores'])  # B,num_proposal
    sem_cls_probs = softmax(end_points[f'{prefix}sem_cls_scores'].asnumpy())  # B,num_proposal,10
    ...
    pred_center_upright_camera = flip_axis_to_camera(pred_center.asnumpy())
    for i in range(bsize):
        for j in range(num_proposal):
            heading_angle = config_dict['dataset_config'].class2angle( \
                pred_heading_class[i, j].asnumpy(), pred_heading_residual[i, j].asnumpy())
            if size_cls_agnostic:
                box_size = pred_size[i, j].asnumpy()
            else:
                box_size = config_dict['dataset_config'].class2size( \
                    int(pred_size_class[i, j].asnumpy()), pred_size_residual[i, j].asnumpy())
    ...
    if config_dict['remove_empty_box']:
        ...
        batch_pc = end_points['point_clouds'].asnumpy()[:, :, 0:3]  # B,N,3
        for i in range(bsize):
            pc = batch_pc[i, :, :]  # (N,3)
            for j in range(K):
    ...
    obj_logits = end_points[f'{prefix}objectness_scores'].asnumpy()
...
def parse_groundtruths(end_points, config_dict, size_cls_agnostic):
    ...
    gt_center_upright_camera = flip_axis_to_camera(center_label[:, :, 0:3].asnumpy())
    for i in range(bsize):
        for j in range(K2):
            if box_label_mask[i, j] == 0: continue
            heading_angle = config_dict['dataset_config'].class2angle(heading_class_label[i, j].asnumpy(),
                                                                      heading_residual_label[
                                                                          i, j].asnumpy())
            if size_cls_agnostic:
                box_size = size_gts[i, j].asnumpy()
            else:
                box_size = config_dict['dataset_config'].class2size(int(size_class_label[i, j].asnumpy()),
                                                                    size_residual_label[i, j].asnumpy())
    ...
    batch_gt_map_cls = []
    for i in range(bsize):
        batch_gt_map_cls.append([(sem_cls_label[i, j].asnumpy().item(), gt_corners_3d_upright_camera[i, j]) for j in
